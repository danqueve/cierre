...
$empleados_admin = ['Alejandro', 'Lourdes', 'Maxi'];
...
<body>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-box">
            ...
            <div class="modal-actions">
                ...
                <a id="confirmDeleteBtn" href="#" class="modal-btn modal-btn-confirm">Sí, eliminar</a>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="card">
            ...
            <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:10px; margin-bottom:1.5rem;">
                <div>
                    <h2 style="margin:0; font-size:1.4rem; font-weight:800; color:var(--text-main);">Historial de Liquidaciones</h2>
                    <p style="margin:4px 0 0; font-size:0.85rem; color:var(--text-muted);">Cobranzas registradas en el sistema</p>
                </div>
                <a href="cargar.php" class="btn btn-primary" style="display:flex; align-items:center; gap:7px; font-size:0.88rem;">
            ...
            <div style="overflow-x: auto;">
                <table class="hist-table" id="histTable">
                    ...
                    <tbody>
                        ...
                        ?>
                        <tr>
                            <td data-label="Fecha" style="font-weight:600;">
                                <?= date('d/m/Y', strtotime($c['fecha_inicio'])) ?>
                                <span style="color:var(--text-muted); font-size:0.82em; font-weight:normal; margin-left:5px;">(Lunes)</span>
                            ...
                            <td data-label="Total" class="text-green" style="font-weight:700; font-size:1.05rem;">
                                <?= formatCurrency($c['total_recaudado']) ?>
                            </td>
                            <td data-label="Acciones" style="text-align:center;">
                                <div style="display:flex; gap:8px; justify-content:center;">
                                    <a href="ver_cierre.php?id=<?= $c['id'] ?>" target="_blank" class="act-btn act-btn-view" title="Ver Liquidación">
                                        <i data-lucide="file-text"></i> Ver
                                    </a>
                                    <button 
                                        class="act-btn act-btn-delete"
                                        onclick="openModal('eliminar_cierre.php?id=<?= $c['id'] ?>&token=<?= csrf_token() ?>')"
                                        title="Eliminar">
    ...
    <script>
        ...
        // Notificaciones de estado desde URL
        const urlParams = new URLSearchParams(window.location.search);
        const status = urlParams.get('status');
        if (status === 'deleted') showToast('Liquidación eliminada correctamente.', 'success');
        if (status === 'error') showToast('Error: ' + (urlParams.get('msg') || 'Acción fallida'), 'error');
